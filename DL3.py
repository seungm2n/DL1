        ...
        - 가장 간단한 모델로 이차원 평면상에 있는 직선 방정식
            - y = mx + b
                ...
                - b : x와 관계없이 y에 영향을 주는 값, 절편(intercept)임
                        회귀선을 위 또는 앙래로 얼마나 평행이동 시킬지 결정함
        - 선형 회귀 알고리즘
...
'''
import numpy as np
# 사이킷런을 코드에서 가져오기 위해서 sklearn이라는 이름으로 가져와야 함
...
# 입력 데이터 집합X
X = [[163],[179],[166],[169],[171]]               # 2차원 리스트
y = [54,63,57,56,58]                              # 정답
regr.fit(X,y)                                     # 데이터ㅓ 사용하여 훈련(학습)하기
...
# 167인 이순신의 키를 넣어서 그 추정값을 출력하기
lee = [[167]]
result = regr.predict(lee)
print('이순신의 키가 {}cm 이므로 몸무게는 {}kg으로 추청됨.'.format(\
                lee, result.round(1) ))
